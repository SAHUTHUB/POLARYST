...
from assets.html_elements import *
...
# Define the regex pattern for invalid filename characters  ✅ เพิ่มบรรทัดนี้แล้ว - ตัวแปรที่หายไป
valid_filename_regex_pattern = r'[\/:*?"<>|]'  # Matches common invalid characters in filenames
...
async def serialize_conversation(conversation: List[Dict]) -> tuple[str, List[str]]: # ✅ type hint *ถูกต้อง*: tuple[str, List[str]]
    ...
    return "\n".join(messages) + "\n", list(set(all_references)) # ✅ Return *2 ค่า*: Serialized Conversation String และ List ของ References
