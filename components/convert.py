import sys  # ✅ เปลี่ยนจาก import platform เป็น import sys และต้องมีบรรทัดนี้!
import os
import asyncio
from pypdf import PdfWriter
from pyppeteer import launch
from common.utils import configs
from components.functions import get_module_path
...
async def generate_pdf_from_html(page, html_file_path, pdf_options, css_to_inject=None):
    ...
    print(f"Path ของไฟล์ HTML ที่กำลังจะเปิด: {html_file_path}") # ✅ บรรทัด Debug ที่เพิ่มเข้ามา
    await page.goto(f"file:///{html_file_path}", {'waitUntil': 'networkidle0'}) # ✅ แก้เป็น file:///... (สังเกต /// สามตัว)
    if css_to_inject:
...
async def html_to_pdf(html_file_executive_summary, html_file_content, html_file_title, html_file_disclaimer,
    ...
    chrome_executable = None  # ✅ กำหนด chrome_executable เป็น None ก่อน if/elif/else
    if sys.platform == 'darwin':  # macOS ✅ ใช้ sys.platform ในการตรวจสอบ OS
        chrome_executable = '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome' # ✅ กำหนด path สำหรับ macOS
    elif sys.platform == 'win32':  # Windows ✅ ใช้ sys.platform ในการตรวจสอบ OS
        chrome_executable = 'C:/Program Files/Google/Chrome/Application/chrome.exe' # ✅ กำหนด path สำหรับ Windows
    else:  # Linux หรือ OS อื่นๆ (optional: เพิ่ม path สำหรับ Linux หรือ OS อื่นๆ ถ้าต้องการ)
        chrome_executable = '/usr/bin/google-chrome' # ✅ กำหนด path สำหรับ Linux (ตัวอย่าง)

    # ✅ เรียก Pyppeteer อย่างถูกต้อง โดยใช้ chrome_executable ที่กำหนดค่าแล้ว
    browser = await launch(
        executablePath=chrome_executable, # ✅ ใช้ตัวแปร chrome_executable ที่กำหนดค่าแล้ว
        headless=True,
        args=['--no-sandbox'],
        handleSIGINT=False,
        handleSIGTERM=False,
